...
/**
 * After ACES, near-black void is transparent (backdrop shows). Only brighter
 * glow gets alpha — dim bloom haze must not veil the sky.
 */
const VOID_BLACK_LUMA = float(0.14)
const VOID_GLOW_LUMA = float(0.48)
...
export function applyFirmamentOverlayOutput(
  ...
  const glowAlpha = smoothstep(VOID_BLACK_LUMA, VOID_GLOW_LUMA, luminance(plate.rgb as never))
